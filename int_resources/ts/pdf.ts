...
function inputInfo(elem, outputVal){
    let infoElem:HTMLParagraphElement | null | undefined = elem?.parentNode?.querySelector(".input-info");
...
function pdfTextSize(){
    ...
    const elem:HTMLInputElement | null = txtFontSize;
...
function pdfHeadingSize(){
    ...
    const elem:HTMLInputElement | null = headingFontSize;
...
// Elements
const mainDesignDivs:NodeListOf<Element> = document.querySelectorAll(".pdf-main-design");
const baseMainLvlDesign:number = 3;

const mainHeadContent:string = "Heading";
const mainTxtContent:string = "&#x2022; Text";
...
mainDesignDivs.forEach(function(designDiv){
    ...
    for (let i = 0; i < 2; i++){
        ...
        // Add mainInner div
        const mainInner:HTMLElement = document.createElement("div");
        mainInner.classList.add("pdf-main-inner");
        designDiv.appendChild(mainInner);

        // Add secDesignElem div
        const secDesignElem:HTMLElement = document.createElement("div");
