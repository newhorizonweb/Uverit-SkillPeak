...
// Set visit counter
let visit = localStorage.getItem('visit') ? parseInt(localStorage.getItem('visit')) : null;
...
let dmState = JSON.parse(localStorage.getItem("dm-checkbox") || "null");
if (dmState !== null) {
...
if (dmBtn !== null) {
    dmBtn.addEventListener("click", function () {
        localStorage.setItem("dm-checkbox", dmBtn.checked.toString());
        const dmState = JSON.parse(localStorage.getItem("dm-checkbox") || "null");
        if (dmState !== null) {
            bodyElem.classList.toggle("dark-mode", dmState);
        }
    });
